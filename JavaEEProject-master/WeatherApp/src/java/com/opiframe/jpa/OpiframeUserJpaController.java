...
import com.opiframe.jpa.exceptions.IllegalOrphanException;
...
public class OpiframeUserJpaController implements Serializable {
    ...
    public void edit(OpiframeUser opiframeUser) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        EntityManager em = null;
        try {
            utx.begin();
            em = getEntityManager();
            ...
            for (DailyWeather dailyWeatherCollectionOldDailyWeather : dailyWeatherCollectionOld) {
                if (!dailyWeatherCollectionNew.contains(dailyWeatherCollectionOldDailyWeather)) {
                    ...
                    illegalOrphanMessages.add("You must retain DailyWeather " + dailyWeatherCollectionOldDailyWeather + " since its userId field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            Collection<DailyWeather> dailyWeatherCollectionOrphanCheck = opiframeUser.getDailyWeatherCollection();
            for (DailyWeather dailyWeatherCollectionOrphanCheckDailyWeather : dailyWeatherCollectionOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This OpiframeUser (" + opiframeUser + ") cannot be destroyed since the DailyWeather " + dailyWeatherCollectionOrphanCheckDailyWeather + " in its dailyWeatherCollection field has a non-nullable userId field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
